const app = Vue.createApp({
  data() {
    return {
    gameList: [
      {
        name: "Apex Legends",
        img: "../img/online_game/apex.jpg",
        url: "https://www.ea.com/ja-jp/games/apex-legends",
      },
      // {
      //   name: "G123",
      //   img: "../img/online_game/G123.jpg",
      //   url: "https://g123.jp/?lang=ja",
      // },
      {
        name: "LOST ARK",
        img: "../img/online_game/LA.jpg",
        url: "https://lostark.pmang.jp/",
      },
      {
        name: "VALORANT",
        img: "../img/online_game/valorant.jpg",
        url: "https://playvalorant.com/ja-jp/",
      },
      // {
      //   name: "War Thunder",
      //   img: "../img/online_game/WT.jpg",
      //   url: "http://www.nepoca.com/title/warthunder/index.html",
      // },
      {
        name: "ドラゴンクエストX",
        img: "../img/online_game/DQ10.jpg",
        url: "http://www.nepoca.com/dqx/",
      },
      {
        name: "FINAL FANTASY XIV",
        img: "../img/online_game/FF14.jpg",
        url: "https://jp.finalfantasyxiv.com/lodestone/",
      },
      {
        name: "PSO2 ニュージェネシス",
        img: "../img/online_game/PSO2.jpg",
        url: "https://pso2.jp/players/",
      },
      {
        name: "リネージュ",
        img: "../img/online_game/lineage.jpg",
        url: "https://www.ncsoft.jp/lineage/",
      },
      {
        name: "リネージュ2",
        img: "../img/online_game/lineage2.jpg",
        url: "https://www.ncsoft.jp/lineage2/",
      },
      {
        name: "タワーオブアイオン",
        img: "../img/online_game/TOA.jpg",
        url: "https://www.ncsoft.jp/aion/",
      },
      {
        name: "マビノギ",
        img: "../img/online_game/mabinogi.jpg",
        url: "https://mabinogi.nexon.co.jp/",
      },
      {
        name: "ブレイドアンドソウル",
        img: "../img/online_game/B&S.jpg",
        url: "https://event2.ncsoft.jp/1.0/bns/landing/",
      },
      {
        name: "黒い砂漠",
        img: "../img/online_game/kurosaba.jpg",
        url: "https://www.jp.playblackdesert.com/",
      },
      {
        name: "ガンダムジオラマフロント",
        img: "../img/online_game/GZF.jpg",
        url: "https://gdf.bandainamco-ol.jp/",
      },
      {
        name: "World of Tanks",
        img: "../img/online_game/WOT.jpg",
        url: "https://worldoftanks.asia/ja/",
      },
      {
        name: "World of Warships",
        img: "../img/online_game/WOW.jpg",
        url: "https://worldofwarships.asia/ja/content/game/",
      },
      {
        name: "Alliance of Valiant Arms",
        img: "../img/online_game/ALL.jpg",
        url: "https://ava.pmang.jp/lp/",
      },
      {
        name: "雀龍門",
        img: "../img/online_game/janryumon.jpg",
        url: "https://www.ncsoft.jp/janryumon/",
      },
      {
        name: "777TOWN.net",
        img: "../img/online_game/777.jpg",
        url: "https://www.777town.net/",
      },
      // {
      //   name: "デスティニーチャイルド",
      //   img: "../img/online_game/DC.jpg",
      //   url: "http://www.nepoca.com/title/destinychild/index.html",
      // },
      // {
      //   name: "DOAX3",
      //   img: "../img/online_game/DOAX3.jpg",
      //   url: "http://www.nepoca.com/title/doax-venusvacation/",
      // },
      {
        name: "DMM GAMES",
        img: "../img/online_game/DMMgame.jpg",
        url: "https://games.dmm.com/",
      },
      {
        name: "ブラウザゲーム for ネットカフェ",
        img: "../img/online_game/Bgame.jpg",
        url: "http://www.nepoca.com/browser/",
      },
    ],
  };
},
});
