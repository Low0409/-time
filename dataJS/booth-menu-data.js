const don = Vue.createApp({
  data() {
    return {
      menulist: [
        {
          name: "ロコモコ丼",
          price: "528",
          tax_price: "580",
          img: "./img/menu/Booth/don/RokomokoBowl.png",
          description: "Rokomoko Bowl",
        },
        {
          name: "月見ソースカツ丼",
          price: "528",
          tax_price: "580",
          img: "./img/menu/Booth/don/tukimiBowl.png",
          description: "Fried Pork & egg Rice Bowl",
        },
        {
          name: "カツとじ丼",
          price: "528",
          tax_price: "580",
          img: "./img/menu/Booth/don/Pork&EggBowl.png",
          description: "Pork & Egg Rice Bowl",
        },
        {
          name: "唐揚げ親子丼",
          price: "528",
          tax_price: "580",
          img: "./img/menu/Booth/don/Chicken&EggRiceBowl.png",
          description: "Chicken & Egg Rice Bowl",
        },
        {
          name: "スタミナ丼",
          price: "528",
          tax_price: "580",
          img: "./img/menu/Booth/don/StaminaBowl.png",
          description: "Stamina Bowl",
        },
        {
          name: "エビピラフ",
          price: "437",
          tax_price: "480",
          img: "./img/menu/Booth/don/ShrimpPiraf.png",
          description: "Shrimp Piraf",
        },
        {
          name: "本格炒めチャーハン",
          price: "437",
          tax_price: "480",
          img: "./img/menu/Booth/don/ChineeStyleFriedRice.png",
          description: "Chinee Style Fried Rice",
        },
      ],
    };
  },
});

const curry = Vue.createApp({
  data() {
    return {
      menulist: [
        {
          name: "オムハヤシ",
          price: "619",
          tax_price: "680",
          img: "./img/menu/Booth/curry/OmHayashi.png",
          description: "Omelette Hayashi",
        },
        {
          name: "ビーフカレー",
          price: "437",
          tax_price: "480",
          img: "./img/menu/Booth/curry/BeefCurry.png",
          description: "Beef Curry",
        },
        {
          name: "ハヤシライス",
          price: "437",
          tax_price: "480",
          img: "./img/menu/Booth/curry/HayashiRice.png",
          description: "Hayashi Rice",
        },
        {
          name: "ビーフカレーオムライス",
          price: "619",
          tax_price: "680",
          img: "./img/menu/Booth/curry/RiceOmelette&BeefCurry.png",
          description: "Rice Omelette & Beef Curry",
        },
        {
          name: "ボルガライス",
          price: "719",
          tax_price: "780",
          img: "./img/menu/Booth/curry/BorgaRice.png",
          description: "Borga Rice",
        },
        {
          name: "たっぷりビーフのドリア",
          price: "528",
          tax_price: "580",
          img: "./img/menu/Booth/curry/BeefDoria.png",
          description: "Beef Doria",
        },
        {
          name: "ケチャップオムライス",
          price: "564",
          tax_price: "620",
          img: "./img/menu/Booth/curry/KetchupRiceOmelette.png",
          description: "Kechap Rice Omelette",
        },
      ],
    };
  },
});

const plate = Vue.createApp({
  data() {
    return {
      menulist: [
        {
          name: "とんかつプレート",
          price: "619",
          tax_price: "680",
          img: "./img/menu/Booth/Plate/tonkatu.png",
          description: "Pork cutlet Plate",
        },
        {
          name: "唐揚げプレート",
          price: "619",
          tax_price: "680",
          img: "./img/menu/Booth/Plate/karaage.png",
          description: "Deep-fried chiken Plate",
        },
        {
          name: "ハンバーグプレート",
          price: "619",
          tax_price: "680",
          img: "./img/menu/Booth/Plate/hamburg.png",
          description: "Hamburg Plate",
        },
        {
          name: "焼肉プレート",
          price: "619",
          tax_price: "680",
          img: "./img/menu/Booth/Plate/yakiniku.png",
          description: "Yakiniku Plate",
        },
      ],
    };
  },
});

const pasta = Vue.createApp({
  data() {
    return {
      menulist: [
        {
          name: "カルボナーラ",
          price: "528",
          tax_price: "580",
          img: "./img/menu/Booth/Pasta/kalbo.png",
          description: "Carbonara",
        },
        {
          name: "ナポリタン",
          price: "528",
          tax_price: "580",
          img: "./img/menu/Booth/Pasta/napoli.png",
          description: "Napolitan Spaghetti",
        },
        {
          name: "焼きそば",
          price: "410",
          tax_price: "450",
          img: "./img/menu/Booth/Pasta/yakisoba.png",
          description: "Fried Noodle",
        },
        {
          name: "ボロネーゼ",
          price: "528",
          tax_price: "580",
          img: "./img/menu/Booth/Pasta/bolo.png",
          description: "Bolognese",
        },
        {
          name: "オムソース焼きそば",
          price: "591",
          tax_price: "650",
          img: "./img/menu/Booth/Pasta/omusoba.png",
          description: "Fried Noodle Omelette",
        },
        {
          name: "きつねうどん",
          price: "591",
          tax_price: "650",
          img: "./img/menu/Booth/Pasta/kitsuneudon.png",
          description: "Kitsune Udon",
        },
        {
          name: "チゲうどん",
          price: "591",
          tax_price: "650",
          img: "./img/menu/Booth/Pasta/tigeudon.png",
          description: "Chige Udon",
        },
        {
          name: "とんこつラーメン",
          price: "591",
          tax_price: "650",
          img: "./img/menu/Booth/Pasta/tonkoturamen.png",
          description: "Tonkotsu Ramen",
        },
        {
          name: "しょうゆラーメン",
          price: "591",
          tax_price: "650",
          img: "./img/menu/Booth/Pasta/syoyuramen.png",
          description: "Soy sauce Ramen",
        },
      ],
    };
  },
});

const pan = Vue.createApp({
  data() {
    return {
      menulist: [
        {
          name: "ミックスピザ",
          price: "528",
          tax_price: "580",
          img: "./img/menu/Booth/Bread&Pizza/mixpizza.png",
          description: "Mix Pizza",
        },
        {
          name: "ロースカツサンド",
          price: "500",
          tax_price: "550",
          img: "./img/menu/Booth/Bread&Pizza/katuSandwitch.png",
          description: "Deep-fried Sirloin Pork Sandwitch",
        },
        {
          name: "味噌カツサンド",
          price: "500",
          tax_price: "550",
          img: "./img/menu/Booth/Bread&Pizza/katuSandwitch.png",
          description: "Miso katsu Sandwitch",
        },
        {
          name: "トースト",
          price: "182",
          tax_price: "200",
          img: "./img/menu/Booth/Bread&Pizza/toast.png",
          description: "Toast",
        },
        {
          name: "ミニサラダ",
          price: "255",
          tax_price: "280",
          img: "./img/menu/Booth/Bread&Pizza/salada.png",
          description: "Mini Salad",
        },
      ],
    };
  },
});

const side = Vue.createApp({
  data() {
    return {
      menulist: [
        {
          name: "フライドポテト",
          price: "255",
          tax_price: "280",
          img: "./img/menu/Booth/side-menu/flyedpoteto.png",
          description: "French Fries",
        },
        {
          name: "メガポテト",
          price: "455",
          tax_price: "500",
          img: "./img/menu/Booth/side-menu/megapoteto.png",
          description: "Mega French Fries",
        },
        {
          name: "若鶏の唐揚げ(6個)",
          price: "346",
          tax_price: "380",
          img: "./img/menu/Booth/side-menu/karaage.png",
          description: "Deep-fried Chicken",
        },
        {
          name: "ポテト&唐揚げ",
          price: "346",
          tax_price: "380",
          img: "./img/menu/Booth/side-menu/potetokaraage.png",
          description: "French Fries & Deep-fried Chicken",
        },
        {
          name: "ポテト&唐揚げ&ソーセージ",
          price: "437",
          tax_price: "480",
          img: "./img/menu/Booth/side-menu/potekarasou.png",
          description: "French Fries & Deep-fried Chicken & Sausages",
        },
        {
          name: "ポテト&ナゲット",
          price: "319",
          tax_price: "350",
          img: "./img/menu/Booth/side-menu/poteto&naget.png",
          description: "French Fries & Naggets",
        },
        {
          name: "ハッシュドポテト",
          price: "228",
          tax_price: "250",
          img: "./img/menu/Booth/side-menu/hashed.png",
          description: "Hashed Potato",
        },
        {
          name: "ハッシュドポテト&ソーセージ",
          price: "346",
          tax_price: "380",
          img: "./img/menu/Booth/side-menu/hashedsou.png",
          description: "Hashed Potato & Sausages",
        },
        {
          name: "チキンナゲット",
          price: "319",
          tax_price: "350",
          img: "./img/menu/Booth/side-menu/naget.png",
          description: "Chiken Nuggets",
        },
        {
          name: "たこ焼き",
          price: "346",
          tax_price: "380",
          img: "./img/menu/Booth/side-menu/takoyaki.png",
          description: "Takoyaki",
        },
      ],
    };
  },
});

const sweets = Vue.createApp({
  data() {
    return {
      menulist: [
        {
          name: "パンケーキ",
          price: "319",
          tax_price: "350",
          img: "./img/menu/Booth/Sweets/pancake.png",
          description: "Pancake",
        },
        {
          name: "スイートパンケーキ",
          price: "364",
          tax_price: "400",
          img: "./img/menu/Booth/Sweets/sweetpancake.png",
          description: "Sweet Pancake",
        },
        {
          name: "フォンダンショコラ",
          price: "455",
          tax_price: "500",
          img: "./img/menu/Booth/Sweets/chocolat2.png",
          description: "Fondant Chocolat",
        },
        {
          name: "フレンチトースト",
          price: "455",
          tax_price: "500",
          img: "./img/menu/Booth/Sweets/frenchtoast.png",
          description: "French Toast",
        },
      ],
    };
  },
});
